...
class MyRobotHockeyEnv(MujocoEnv):
    ...
    def _compute_reward(self):
        ...
        dist_to_puck = np.linalg.norm(ee_pos[:2] - puck_pos[:2])

        # --- 近接報酬 (近いほど高く) ---
        prox_limit = 0.5
        if dist_to_puck < prox_limit:
            reward += 1.0 * (1.0 - dist_to_puck / prox_limit)

        # --- 距離ペナルティ (遠いほど少しずつマイナス) ---
        reward -= 0.01 * dist_to_puck
